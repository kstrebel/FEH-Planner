...
<td style="border:unset;border-left:2px var(--th-color1) solid;">10000100700
    ...
    try:
        ...
        u_id = u_id[u_id.find("(") + 1 : u_id.find(")")]
        u_name = unitInfo.find(style="font-size:160%;").get_text()
        u_epithet = unitInfo.find(style="font-size:90%").get_text()
        u_imageAddress = ""
        u_entry1 = ""
        u_entry2 = ""
        u_movement = ""
...
def dataAfterTh(table, header):
    element = table.find("th", string=re.compile(header)).find_next_sibling("td")
    return element.getText("|", strip=True)
